...
- `mobile-porter` (`.claude/agents/mobile-porter.md`) — subagente de solo lectura que audita si **un juego puntual** con motor real está bien portado a mobile/touch, usando `specs/08-controles-tactiles.md` (breakpoint 840px, contrato de `TouchControls`, `pressVirtualKey`/`releaseVirtualKey`/`movePaddleTo`, `touch-action: none`, auto-repeat, z-index del overlay contra el HUD) como checklist normativo. Pensado sobre todo para correr después de portar un juego nuevo, antes de darlo por terminado; también sirve como chequeo de regresión sobre Asteroides/Caída/Bloque Buster. Si no le dicen a qué juego, pregunta antes de leer código, listando los juegos elegibles. No audita Nav/Biblioteca/Salón/Home/Auth — solo juegos. Nunca escribe ni corrige código — el fix queda para que el usuario lo pida explícitamente. Mantiene memoria persistente en `mobile-porter/memory.md` y publica una tabla curada en `references/games-mobile-review.md`. Invocalo explícitamente con "usa mobile-porter" o "revisá el mobile de `<juego>`".
- `game-performance-booster` (`.claude/agents/game-performance-booster.md`) — subagente que revisa el costo de dibujo por frame del motor de **un juego puntual** dado por `id`/slug de catálogo, usando `specs/10-performance-frogger.md` como referencia normativa: detecta el patrón de `ctx.shadowBlur`/`ctx.shadowColor` fijado por entidad individual en vez de agrupado por lote homogéneo (el mismo que causó el lag reportado en Frogger en un Huawei de gama media) y, si lo encuentra, lo refactoriza directamente replicando el patrón ya validado en `frogger-engine.ts` (`drawEntityGlow`/`drawEntityDetail`), corriendo `npm run lint` y `npm run build` al final. Otros hallazgos de performance más riesgosos (tablero estático redibujado sin caché, reasignación incondicional de arrays por frame, colisión O(n²), etc.) los reporta pero nunca los autofija — mismo criterio de "quick win primero" que usó spec 10. Si no le dicen a qué juego, pregunta antes de tocar código, listando los juegos elegibles (con motor real: hoy `asteroides`, `caida`, `bloque-buster`, `ranaria`). Un placeholder sin motor real señalado como objetivo queda bloqueado, no implementado. Mantiene memoria persistente entre corridas en `game-performance-booster/memory.md` (historial detallado interno) y publica una tabla curada en `references/games-performance-review.md`. Invocalo explícitamente con "usa game-performance-booster" o "revisá el performance de `<juego>`".
